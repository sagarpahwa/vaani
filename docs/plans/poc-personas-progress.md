...
| P3.4 | `compute_style_match` (0–1) from band/pause/expressiveness distance — *two-directional*: overshooting the persona docks it | DONE | `pending` | `pytest test_persona_style_match.py` (6 passed): matched → high, far → low, overexpressive lowers match for monotone persona |
