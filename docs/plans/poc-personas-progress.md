...
| P4.8a | **Bug found during P4.8:** `practice_sessions` `$jsonSchema` `mode` enum was `[guided, user_script]` — real Mongo rejected `mode:"persona"` with a 500 on session create. mongomock doesn't enforce `$jsonSchema`, so unit tests (and CI) never caught it; only the live demo path hits it. Fix: add `persona` to the enum in `services/api/db/schemas/practice_sessions.json` + update the static guard in `test_schemas_poc.py` that pinned the enum to two values | DONE | `pending` | `make poc-api-test` green (195); persona session creates + scores against real Mongo (style_match 0.77) |
