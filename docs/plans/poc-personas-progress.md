...
| P3.8 | **New** persona golden fixtures under `services/api/tests/golden/` (`persona_dataset.json` + `test_persona_golden.py`; `persona_model_quality` floor in `quality-baseline.poc.json`); Mode A/B `dataset.json` untouched | DONE | `pending` | `make poc-api-test` golden green (193 passed, 3 persona cases: jobs-clean 0.93/style 0.81, buffett-clean 0.86/style 0.59, jobs-skip drags coverage); Mode A/B golden byte-unchanged in `git diff` |
