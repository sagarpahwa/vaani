...
| P3.7 | Persona version constants + `persona_version_stamp()` in `domain/versions.py` (distinct from Mode A/B so goldens stay independent) | DONE | `pending` | `pytest test_versions.py` (3 passed): same 4 keys, all values differ from Mode A/B |
