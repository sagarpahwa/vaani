...
| P4.5 | `app/src/app/index.tsx` — primary accent-bordered "Practice with a great speaker" card (20 LEGENDS badge) → `/personas`, above Guided Practice | DONE | `pending` | `make poc-app-test` green. Note: Expo typed-routes file `.expo/types/router.d.ts` is gitignored + regenerated by the dev server — a stale local copy will fail `tsc` on a new route's `Href` until `npx expo start` rewrites it (CI checks out fresh, so it's unaffected) |
